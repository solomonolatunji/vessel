...
func (s *Store) CreateDatabase(db *types.DatabaseConfig) error {
	...
	_, err = s.db.Exec(`INSERT INTO databases (
		id, name, engine, version, port, username, encrypted_password, database_name, volume_path, container_id, status, internal_dns, external_dns, created_at, updated_at
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		db.ID, db.Name, db.Engine, db.Version, db.Port, db.Username, encryptedPassword, db.DatabaseName, db.VolumePath, db.ContainerID, db.Status, db.InternalDNS, db.ExternalDNS, db.CreatedAt, db.UpdatedAt)
	return err
}
...
func (s *Store) GetDatabase(id string) (*types.DatabaseConfig, error) {
	...
	err := s.db.QueryRow(`SELECT id, name, engine, version, port, username, encrypted_password, database_name, volume_path, COALESCE(container_id, ''), status, COALESCE(internal_dns, ''), COALESCE(external_dns, ''), created_at, updated_at
...
func (s *Store) ListDatabases() ([]types.DatabaseConfig, error) {
	rows, err := s.db.Query(`SELECT id, name, engine, version, port, username, encrypted_password, database_name, volume_path, COALESCE(container_id, ''), status, COALESCE(internal_dns, ''), COALESCE(external_dns, ''), created_at, updated_at FROM databases ORDER BY created_at ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var db types.DatabaseConfig
		var encryptedPassword string
		if err := rows.Scan(&db.ID, &db.Name, &db.Engine, &db.Version, &db.Port, &db.Username, &encryptedPassword, &db.DatabaseName, &db.VolumePath, &db.ContainerID, &db.Status, &db.InternalDNS, &db.ExternalDNS, &db.CreatedAt, &db.UpdatedAt); err != nil {
			return nil, err
		}
