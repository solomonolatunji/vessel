...
// initAppServiceTable creates the app_services table if it doesn't already exist.
...
func (s *Store) CreateAppService(service *types.AppServiceConfig) error {
	...
	query := `INSERT INTO app_services (
		id, project_id, environment_id, name, repository_url, branch, build_command,
		start_command, dockerfile_path, internal_port, domain, env_vars_count,
		auto_deploy_webhook, cpu_request, memory_limit_mb, health_check_path,
		status, container_id, created_at, updated_at
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`

	_, err := s.db.Exec(query,
		service.ID, service.ProjectID, service.EnvironmentID, service.Name, service.RepositoryURL, service.Branch,
		service.BuildCommand, service.StartCommand, service.DockerfilePath, service.InternalPort, service.Domain,
		service.EnvVarsCount, service.AutoDeployWebhook, service.CPURequest, service.MemoryLimitMB, service.HealthCheckPath,
		service.Status, service.ContainerID, service.CreatedAt, service.UpdatedAt,
	)
	if err != nil {
...
func (s *Store) GetAppService(id string) (*types.AppServiceConfig, error) {
	...
	query := `SELECT id, project_id, environment_id, name, repository_url, branch, build_command,
		start_command, dockerfile_path, internal_port, domain, env_vars_count,
		auto_deploy_webhook, cpu_request, memory_limit_mb, health_check_path,
		status, container_id, created_at, updated_at FROM app_services WHERE id = ?`

	row := s.db.QueryRow(query, id)
	var app types.AppServiceConfig
	var autoDeploy int
	err := row.Scan(
		&app.ID, &app.ProjectID, &app.EnvironmentID, &app.Name, &app.RepositoryURL, &app.Branch,
		&app.BuildCommand, &app.StartCommand, &app.DockerfilePath, &app.InternalPort, &app.Domain,
		&app.EnvVarsCount, &autoDeploy, &app.CPURequest, &app.MemoryLimitMB, &app.HealthCheckPath,
		&app.Status, &app.ContainerID, &app.CreatedAt, &app.UpdatedAt,
	)
...
func (s *Store) ListAppServicesByEnvironment(environmentID string) ([]*types.AppServiceConfig, error) {
	...
	query := `SELECT id, project_id, environment_id, name, repository_url, branch, build_command,
		start_command, dockerfile_path, internal_port, domain, env_vars_count,
		auto_deploy_webhook, cpu_request, memory_limit_mb, health_check_path,
		status, container_id, created_at, updated_at FROM app_services WHERE environment_id = ? ORDER BY created_at ASC`
...
func (s *Store) ListAppServicesByProject(projectID string) ([]*types.AppServiceConfig, error) {
	...
	query := `SELECT id, project_id, environment_id, name, repository_url, branch, build_command,
		start_command, dockerfile_path, internal_port, domain, env_vars_count,
		auto_deploy_webhook, cpu_request, memory_limit_mb, health_check_path,
		status, container_id, created_at, updated_at FROM app_services WHERE project_id = ? ORDER BY created_at ASC`
