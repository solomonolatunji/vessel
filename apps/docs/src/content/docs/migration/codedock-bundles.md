---
title: Codedock Bundles
description: Export and import encrypted Codedock migration bundles between servers.
---

Codedock bundles move a Codedock instance from one server to another. A bundle is a Codedock-to-Codedock migration format.

## Export a Bundle

Open System Settings, choose Migration, and export the instance. You must enter a passphrase with at least 8 characters.

Codedock creates a `.codedock` file. The file is an encrypted archive, so keep the passphrase with the same care you would give a database backup key.
...
During export, Codedock creates disk backups for database services and copies those backup files into the bundle.
...
During onboarding on the target server, choose the `.codedock` file and enter the passphrase from the source server.

Codedock decrypts the bundle, validates the manifest, writes managed runtime env, saves system settings, replaces logical data, restores optional files, restores database dumps, reloads Traefik, restores users, and clears auth sessions.

## Runtime Env Merge

Some runtime values must stay local to the target server. On import, Codedock preserves target values for:
