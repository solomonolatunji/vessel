...
Codedock treats deployments as jobs. A service can have many deployments over time, but only one deployment per service can build at once.
...
- Codedock migration import follow-up deployment queueing for restored active app services.
...
Codedock uses that global number as the deployment slot limit. It also prevents two deployments for the same service from running at the same time.
...
For queued deployments, Codedock marks the job aborted before it starts.

For building deployments, Codedock marks the job aborted, asks the active process to stop, then escalates to a force kill if needed. Any temporary containers are cleaned up by the deployment flow.

## Web Hot Swaps

For source-built web services and Docker image web services, Codedock keeps the old container live while the new container proves it can answer on the configured port.
...
If the new container does not become reachable, Codedock keeps the previous service state instead of switching traffic.

## Workers and Static Sites

Worker deployments check that the process stays running. There is no HTTP port probe.

Static site deployments export files from the built image into the static site directory. Codedock verifies that the output contains `index.html` before marking the deployment running.

## Database Deployments

Database deployments are different from app hot swaps. Codedock runs database containers with persistent Docker volumes. Deploying a database service ensures the image, volume, runtime network, public hostname route, Postgres TLS assets, and logical replication settings are in place.
...
If a deployment fails, Codedock provides an **AI Diagnose** button next to the deployment logs. By clicking this, Codedock will:
