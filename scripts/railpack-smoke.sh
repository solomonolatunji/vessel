...
echo "🚂 Vessel — Railpack CI Smoke Test"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
# Replace ghcr.io/vessel/railpack with the actual local build if needed
...
docker run --rm -v "$TEST_DIR:/workspace" -v /var/run/docker.sock:/var/run/docker.sock \
  ghcr.io/vessel/railpack:latest build -t "vessel-$APP_NAME" /workspace || {
    echo "⚠️  Railpack build failed or image unavailable. Skipping execution."
    exit 0
}

echo "🚀 Starting test container..."
CONTAINER_ID=$(docker run -d -p 3000:3000 "vessel-$APP_NAME")
