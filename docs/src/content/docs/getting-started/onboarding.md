...
description: Finish the first-run setup after Codedock is installed on your server.
sidebar:
  order: 2
---
...
You can serve Codedock itself through Traefik by choosing a dashboard domain such as:
...
Codedock keeps the raw `http://IP:8080` URL available as a fallback.

## Configure GitHub

Codedock works best with a GitHub App. Create one with these repository permissions:
...
If you do not want to use a GitHub App, Codedock can also read a `GITHUB_ACCESS_TOKEN` from the server environment. The GitHub App path is better for repository selection and push webhooks.

## Add the Wildcard Root Domain

Set a wildcard root domain when you want Codedock to generate service hostnames automatically:
...
If you have a Cloudflare R2 bucket or token ready, connect R2 in the same step. Codedock can then save database backups to disk, R2, or both.
...
## Import an Codedock Bundle

If you are moving from another Codedock server, use the migration import option during onboarding. Choose the `.codedock` file and enter the passphrase from the source server.

The import restores projects, services, users, domains, environment variables, static output, backup records, backup files, database dumps, Traefik config, and system settings. Codedock clears auth sessions after import, then sends you through the success path so the restored owner account can sign in cleanly.
